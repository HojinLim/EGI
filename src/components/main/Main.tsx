      ...
      if (error) {
        ...
        const postsWithCompleteURLs = data.map((post) => ({
          ...
          image_urls: post.image_urls
            ? post.image_urls.replace(/\[|\]|"/g, "").split(",")
            : [],
        }));
      ...
      if (searchKeyword.trim() === "" || searchKeyword === "") {
        ...
        const filtered = posts.filter(
          (post) =>
            ...
            post.location.toLowerCase().includes(keywordLower)
        );
        setCurrentPage(1);
        ...
        {paginatedData.map((post) => (
          ...
          >
            <S.PostItem>
              ...
              <S.CardPrice>
                ₩ {post.price?.toLocaleString("en-NZ")}원
              </S.CardPrice>

              <S.CardCondition>
                <div>
                  <LikeFilled
                    style={{ color: "#0A3A8D", marginRight: "5px" }}
                  />
                  {jjimCount(post.pid)}
                </div>
                <CONDITION.CoditionBox>
                  <CONDITION.CoditionIscompleted>
                    {post.iscompleted}
                  </CONDITION.CoditionIscompleted>
                  <CONDITION.CoditionProduct condition={post.condition}>
                    {post.condition}
                  </CONDITION.CoditionProduct>
                </CONDITION.CoditionBox>
              </S.CardCondition>
            </S.PostItem>
          </NavLink>
